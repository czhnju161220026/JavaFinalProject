## 12月26日 更新
为了增加游戏的视觉效果，为子弹添加方向这个属性，让子弹可以斜向飞行。<br>
在Attribute包中添加了枚举类型:<br>
**BulletDirection**：这个属性有两个成员变量，代表子弹飞行时在x轴方向和y轴方向的增量，也就是子弹单位时间运动的矢量。它的定义如下：
``` java
@TODO(todo="指示子弹的飞行方向")
...
```
同时在子弹的构造函数中加入方向参数，这样就可以指定子弹飞行的方向了。
